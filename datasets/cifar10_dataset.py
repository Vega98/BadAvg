...
# Modifico questa funzione per la nuova routine "federated_round"
# Commento il codice vecchio
def get_pretraining_cifar10(train_dir, test_dir):

    #train_data = CIFAR10Pair(numpy_file=data_dir + "train.npz", class_type= classes, transform=train_transform)
    train_data = CIFAR10Pair(numpy_file=train_dir, class_type= classes, transform=train_transform)
    #memory_data = CIFAR10Mem(numpy_file=data_dir + "train.npz", class_type= classes, transform=test_transform_cifar10)
    memory_data = CIFAR10Mem(numpy_file=train_dir, class_type= classes, transform=test_transform_cifar10)
    #test_data  = CIFAR10Mem(numpy_file=data_dir + "test.npz", class_type= classes,transform=test_transform_cifar10)
    test_data  = CIFAR10Mem(numpy_file=test_dir, class_type= classes,transform=test_transform_cifar10)

    return train_data, memory_data, test_data


# Qui hanno hardcodato le dimensioni di training_data e testing_data.
# Se vogliamo usarlo con una partizione del cifar10, dobbiamo andare a
# specificare manualmente il numero di esempi di training e testing.
# testing_data_num non è usato dal codice, quindi non andiamo a toccarlo.
# Inoltre, se vogliamo usare il test dataset come shadow dataset, dobbiamo,
# oltre che modificare il numero di esempi di training (10000), anche modificare
# 'train.npz' con 'test.npz' nelle righe 72 e 81.
