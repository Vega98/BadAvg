...
def run_pretraining(
    ...
    cmd = (
        f"{sys.executable} pretraining_encoder.py "
        f"--checkpoint {global_model_path} "
        f"--epochs {epochs} "
        f"--train_dir {dataset_path} "
        f"--mem_dir {mem_dir} "
        f"--test_dir {test_dir} "
        f"--results_dir {output_dir} "
        f"--name model_ft_c{client_id} "
        f"--batch_size 256 "
        f"--pretraining_dataset {pretraining_dataset} "
        f"--gpu {gpu} "
        f"--current_round {current_round} "
        f"--neurotoxin {neurotoxin}"
    )
